// components/calculators/compound-interest-results.tsx
'use client'

import type { CompoundInterestResults } from '@/lib/calculators/compound-interest'
...
export function CompoundInterestResults({ results, annualRate }: Props) {
  const { finalAmount, totalContributed, totalInterest, doublingTime, yearRows } = results

  const interestPct = finalAmount > 0
    ? ((totalInterest / finalAmount) * 100).toFixed(1)
    : '0'

  const visibleRows = yearRows.filter(
    (row) => row.year % 5 === 0 || row.year === 1 || row.year === yearRows.length
  )

  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
      {/* Primary result */}
      <div style={{ padding: '2rem 1.875rem 1.625rem', borderBottom: '1px solid var(--f-border)' }}>
        <p
          style={{
            fontSize: '0.6875rem', fontWeight: 700, letterSpacing: '0.1em',
            textTransform: 'uppercase', color: 'var(--f-blue)', marginBottom: '0.5rem',
          }}
        >
          Final Balance
        </p>
        ...
        <p style={{ fontSize: '0.8125rem', color: 'var(--f-text-muted)', fontWeight: 300 }}>
          {interestPct}% from compound interest
        </p>

        {/* Principal vs interest bar */}
        <div style={{ marginTop: '1.125rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.375rem' }}>
            <span style={{ fontSize: '0.6875rem', color: 'var(--f-text-faint)', fontWeight: 400 }}>Principal + Contributions</span>
            <span style={{ fontSize: '0.6875rem', color: 'var(--f-text-faint)', fontWeight: 400 }}>Interest</span>
          </div>
          <div style={{ background: 'var(--f-border)', borderRadius: '99px', height: '6px', overflow: 'hidden', display: 'flex' }}>
            <div
              style={{
                height: '100%',
                width: `${Math.min(100, (totalContributed / finalAmount) * 100)}%`,
      ...
      >
        {[
          ...
          { label: 'Rule of 72', value: doublingTime !== null ? `${doublingTime.toFixed(1)} yrs to 2×` : '—' },
        ].map((item) => (
          <div key={item.label}>
      ...
      <div style={{ padding: '1.25rem 1.875rem', flex: 1, overflowX: 'auto' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8125rem' }}>
          <thead>
            <tr>
              {['Year', 'Balance', 'Contributed', 'Interest'].map((col) => (
                <th
                  key={col}
                  style={{
                    textAlign: 'left', fontSize: '0.6875rem', fontWeight: 600,
                    color: 'var(--f-text-faint)', letterSpacing: '0.04em', textTransform: 'uppercase',
                    paddingBottom: '0.5rem', borderBottom: '1px solid var(--f-border)', paddingRight: '0.75rem',
                  }}
                >
                  {col}
                </th>
          ...
          <tbody>
            {visibleRows.map((row) => {
              ...
              return (
                <tr key={row.year} style={{ background: isLast ? 'oklch(0.96 0.02 258)' : undefined }}>
                  <td
                    style={{
