// components/calculators/barista-fire-calculator.tsx
'use client'

import { useState, useMemo, useRef } from 'react'
import { Slider } from '@/components/ui/slider'
import { baristaFireSchema, type BaristaFireInputs } from '@/lib/validation/barista-fire-schema'
import { calculateBaristaFire } from '@/lib/calculators/barista-fire'
import { BaristaFireResultsPanel } from './barista-fire-results'
import { BaristaFireChart } from './barista-fire-chart'

const DEFAULTS: BaristaFireInputs = {
  currentAge: 35,
...
export function BaristaFireCalculator() {
  const [inputs, setInputs] = useState<BaristaFireInputs>(DEFAULTS)

  function set<K extends keyof BaristaFireInputs>(key: K, value: BaristaFireInputs[K]) {
    setInputs((prev) => ({ ...prev, [key]: value }))
  }

  const results = useMemo(() => {
    const parsed = baristaFireSchema.safeParse(inputs)
    if (!parsed.success) return null
    return calculateBaristaFire(parsed.data)
  }, [inputs])
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <GroupLabel>Barista FIRE Inputs</GroupLabel>
          ...
          <NumberInput id="totalExpenses" label="Annual Total Expenses" value={inputs.totalExpenses}
            onChange={(v) => set('totalExpenses', v)} prefix="$" min={1000} max={500000} hint="total spending" />
          <NumberInput id="partTimeIncome" label="Annual Part-Time Income" value={inputs.partTimeIncome}
            onChange={(v) => set('partTimeIncome', v)} prefix="$" min={0} max={100000} hint="semi-retirement income" />
          ...
          <SliderInput id="expectedReturn" label="Expected Return" value={inputs.expectedReturn}
            ...
            tooltip="The average annual growth rate you expect your investments to earn. The US stock market has historically returned ~7% after inflation. Higher returns mean reaching Barista FIRE sooner." />
          <SliderInput id="inflationRate" label="Inflation Rate" value={inputs.inflationRate}
            ...
            tooltip="The annual rate at which the cost of living rises. Inflation erodes purchasing power over time. The US long-run average is around 3%." />
          <SliderInput id="withdrawalRate" label="Withdrawal Rate" value={inputs.withdrawalRate}
            ...
            tooltip="How much of your portfolio you withdraw each year to cover expenses not covered by part-time income. The '4% rule' has historically sustained portfolios for 30+ years." />
        </div>

        {/* Results */}
        <div style={{ background: 'oklch(0.985 0.004 258)', display: 'flex', flexDirection: 'column' }}>
          {results ? (
            <BaristaFireResultsPanel
              results={results}
              totalExpenses={inputs.totalExpenses}
      ...
      {results && (
        <div style={{ padding: '1.5rem 1.75rem 1.375rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'center' }}>
              {[
                ...
                { color: 'oklch(0.68 0.15 195)', label: 'Barista FIRE target', dashed: true },
                { color: 'oklch(0.72 0.08 258)', label: 'Full FIRE target', dashed: true },
              ].map((item) => (
          ...
          <BaristaFireChart
            ...
            baristaFireNumber={results.baristaFireNumber}
            fullFireNumber={results.fullFireNumber}
          />
        </div>
      )}
    </div>
