// components/calculators/lean-fire-calculator.tsx
'use client'

import { useState, useMemo, useRef } from 'react'
import { Slider } from '@/components/ui/slider'
import { leanFireSchema, type LeanFireInputs } from '@/lib/validation/lean-fire-schema'
import { calculateLeanFire } from '@/lib/calculators/lean-fire'
import { LeanFireResults } from './lean-fire-results'
import { LeanFireChart } from './lean-fire-chart'

const DEFAULTS: LeanFireInputs = {
  currentAge: 30,
...
function NumberInput({
  ...
  if (prevValueRef.current !== value) {
    ...
    // Only update raw if the parsed value differs (avoid cursor jump on every keystroke)
...
export function LeanFireCalculator() {
  const [inputs, setInputs] = useState<LeanFireInputs>(DEFAULTS)

  function set<K extends keyof LeanFireInputs>(key: K, value: LeanFireInputs[K]) {
    setInputs((prev) => ({ ...prev, [key]: value }))
  }

  const results = useMemo(() => {
    const parsed = leanFireSchema.safeParse(inputs)
    if (!parsed.success) return null
    return calculateLeanFire(parsed.data)
  }, [inputs])
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <GroupLabel>Lean FIRE Inputs</GroupLabel>
          ...
          <NumberInput id="annualLeanExpenses" label="Annual Lean Expenses" value={inputs.annualLeanExpenses}
            onChange={(v) => set('annualLeanExpenses', v)} prefix="$" min={5000} max={100000} hint="frugal spending" />
          ...
          <SliderInput id="expectedReturn" label="Expected Return" value={inputs.expectedReturn}
            ...
            tooltip="The average annual growth rate you expect your investments to earn. The US stock market has historically returned ~7% after inflation. Higher returns mean reaching Lean FIRE sooner, but come with more risk." />
          <SliderInput id="inflationRate" label="Inflation Rate" value={inputs.inflationRate}
            ...
            tooltip="The annual rate at which the cost of living rises. Inflation erodes your purchasing power over time, so your Lean FIRE number must account for it. The US long-run average is around 3%." />
          <SliderInput id="withdrawalRate" label="Withdrawal Rate" value={inputs.withdrawalRate}
            ...
            tooltip="How much of your portfolio you spend each year in retirement. The '4% rule' has historically sustained portfolios for 30+ years. For very long Lean FIRE retirements, consider 3–3.5%." />
        </div>

        {/* Results */}
        <div style={{ background: 'oklch(0.985 0.004 258)', display: 'flex', flexDirection: 'column' }}>
          {results ? (
            <LeanFireResults results={results} annualLeanExpenses={inputs.annualLeanExpenses} />
          ) : (
            <div
              style={{
                flex: 1,
      ...
      {results && (
        <div style={{ padding: '1.5rem 1.75rem 1.375rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'center' }}>
              {[
                ...
                { color: 'oklch(0.68 0.15 195)', label: 'Lean FIRE target', dashed: true },
              ].map((item) => (
          ...
          <LeanFireChart data={results.projectionData} leanFireNumber={results.leanFireNumber} />
        </div>
      )}
    </div>
  )
